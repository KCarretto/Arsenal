...
from testutils import ModelTest, get_session, get_target, create_test_session #pylint: disable=no-name-in-module
...
class SessionModelTest(ModelTest):
    ...
    def test_find_pass(self):
        ...
        session2 = get_session(session1.session_id)
        self.assertEqual(session1, session2)

        self.assertIsNotNone(get_target(session2.target_name))
    ...
    def test_target_status_pass(self):
        ...
        """
        session1 = create_test_session(None, 0)
        target = get_target(session1.target_name)
        self.assertEqual(target.status, SESSION_STATUSES.get('inactive'))

        missing_timer = 2401 + SESSION_CHECK_THRESHOLD
        session2 = create_test_session(target.name, time.time() - missing_timer, 1800, 600)
        self.assertEqual(target.status, SESSION_STATUSES.get('missing'))

        session3 = create_test_session(target.name)
